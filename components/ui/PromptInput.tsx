...
import { ArrowUp, Paperclip, Settings, Shuffle, X, Globe, Check, Upload, Palette, Image as ImageIcon, Link2, Mic, Copy, RefreshCw, ArrowRight } from 'lucide-react';
...
export const PromptInput: React.FC<PromptInputProps> = ({ 
  ...
  const toggleListening = () => {
    if (isListening) {
      // Stop listening (handled by onend in most cases, but forced here)
  ...
  const processImageUpload = async (file: File) => {
    ...
    if (url) {
      ...
      setShowScreenshotModal(false); // Close modal on success
    } else {
  ...
  const handleLogoSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const url = await uploadToImgBB(file);
    if (url) {
      if (onBrandLogoChange) onBrandLogoChange(url);
    } else {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className={`relative group bg-white border border-stone-200 shadow-2xl shadow-stone-200/50 rounded-[2rem] transition-all duration-300 focus-within:shadow-stone-300/50 focus-within:border-stone-300 ${className}`}>
          ...
          <div className="px-6 pt-6 pb-0 flex flex-wrap gap-2">
              {uploadedImage && (
                <div className="relative inline-block">
                  <img 
                    src={uploadedImage} 
                    alt="Uploaded" 
                    className="h-14 w-auto rounded-xl border border-stone-200 shadow-sm"
                  />
                  <button 
                    onClick={clearImage}
                    className="absolute -top-2 -right-2 bg-stone-900 text-white rounded-full p-1 shadow-md hover:bg-stone-700 transition-colors"
                  >
                    <X size={10} />
                  </button>
                </div>
              ...
              {figmaUrl && (
                  <div className="inline-flex items-center gap-2 bg-stone-50 border border-stone-200 rounded-xl px-3 py-1.5 text-xs font-medium text-stone-700 h-10">
                      ...
                      <button onClick={() => onFigmaUrlChange?.('')} className="ml-1 text-stone-400 hover:text-stone-900">
                          <X size={12} />
                      </button>
                  </div>
              )}
              ...
              {youtubeUrl && (
                  <div className="inline-flex items-center gap-2 bg-red-50 border border-red-100 rounded-xl px-3 py-1.5 text-xs font-medium text-red-700 h-10">
                      ...
                      <button onClick={() => onYoutubeUrlChange?.('')} className="ml-1 text-red-300 hover:text-red-700">
                          <X size={12} />
                      </button>
                  </div>
              )}
        ...
        <div className="absolute bottom-3 left-6 right-4 flex justify-between items-center">
          <div className="flex items-center gap-2 relative" ref={settingsRef}>
              ...
              {/* Settings Popover */}
              {showSettings && (
                <div className="absolute bottom-full mb-2 left-0 w-[340px] bg-white border border-stone-200 shadow-xl rounded-2xl p-5 z-20 animate-in fade-in zoom-in-95 duration-200">
                  <div className="flex justify-between items-center mb-4">
                      <h3 className="text-sm font-bold text-stone-900">Brand Identity</h3>
                      <button onClick={() => setShowSettings(false)} className="text-stone-400 hover:text-stone-600">
                          <X size={16} />
                      </button>
                  </div>

                  {/* Section 1: Website (Auto) */}
                  <div className="mb-5">
                      ...
                      <div className="flex items-center gap-3">
                          ...
                          <div className="flex-grow">
                              ...
                              <button 
                                  ...
                                  className="text-xs font-medium text-stone-900 hover:text-pink-600 border-b border-stone-200 hover:border-pink-600 transition-all pb-0.5"
                              >
                      ...
                      <div className="flex items-center gap-3">
                          <div className="relative h-8 w-full rounded-lg bg-stone-50 border border-stone-200 flex items-center px-2 overflow-hidden">
                              <input 
                                  ...
                                  className="absolute left-0 top-0 w-8 h-full opacity-0 cursor-pointer"
                              />
                              <div 
                                  className="w-4 h-4 rounded-full shadow-sm border border-black/10 mr-2"
              ...
              <button className="hidden sm:flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-stone-500 bg-stone-50 border border-stone-100 rounded-full hover:bg-stone-100 transition-colors">
                  <Shuffle size={14} />
                  <span>Auto</span>
              </button>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button 
                ...
                className={`
                  p-2 rounded-xl transition-all active:scale-95 flex items-center justify-center min-w-[40px] min-h-[40px]
                  ${isListening ? 'bg-red-500 text-white animate-pulse' : 'text-stone-400 hover:text-stone-800 hover:bg-stone-50'}
                `}
                title="Voice Input"
              >
                <Mic size={20} />
            ...
            <button 
              onClick={onSubmit}
              disabled={(!value.trim() && !uploadedImage && !figmaUrl && !youtubeUrl) || isLoading || isUploading}
              className="bg-black text-white rounded-full p-2.5 hover:bg-stone-800 disabled:opacity-50 disabled:cursor-not-allowed transition-all active:scale-95 shadow-lg flex items-center justify-center min-w-[40px] min-h-[40px]"
            >
              {isLoading ? (
      ...
      {/* Feature Pills - Compact View */}
      <div className="flex gap-2 px-2 overflow-x-auto pb-2 no-scrollbar scroll-smooth flex-nowrap mask-linear-fade">
        <button 
          onClick={() => setShowScreenshotModal(true)}
          className="flex-shrink-0 bg-white border border-stone-200 rounded-full px-4 py-2 flex items-center gap-2 hover:border-stone-300 hover:shadow-md transition-all active:scale-95 group whitespace-nowrap"
        >
           <ScreenshotIcon className="w-4 h-4 text-pink-500" />
           <span className="text-xs font-semibold text-stone-700 group-hover:text-stone-900">Screenshot to Email</span>
        </button>
        ...
        >
           ...
           <span className="text-xs font-semibold text-stone-700 group-hover:text-stone-900">Figma to Email</span>
        </button>
        ...
        >
           <YoutubeIcon className="w-4 h-4" />
           <span className="text-xs font-semibold text-stone-700 group-hover:text-stone-900">YouTube to Email</span>
        </button>
        ...
        <button 
          ...
          className="flex-shrink-0 bg-white border border-stone-200 rounded-full px-4 py-2 flex items-center gap-2 hover:border-stone-300 hover:shadow-md transition-all active:scale-95 group whitespace-nowrap"
        >
           <PromptBuilderIcon className="w-4 h-4 text-purple-500" color="currentColor" />
           <span className="text-xs font-semibold text-stone-700 group-hover:text-stone-900">Prompt Builder</span>
        </button>
        ...
        <button 
          ...
          className="flex-shrink-0 bg-white border border-stone-200 rounded-full px-4 py-2 flex items-center gap-2 hover:border-stone-300 hover:shadow-md transition-all active:scale-95 group whitespace-nowrap"
        >
           <WebsiteIcon className="w-4 h-4 text-blue-500" />
           <span className="text-xs font-semibold text-stone-700 group-hover:text-stone-900">Website to Email</span>
        </button>
      </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-[2rem] shadow-2xl w-full max-w-lg p-8 animate-in zoom-in-95 fade-in duration-300">
             <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-bold text-stone-900 font-display">Screenshot to Email</h2>
                <button onClick={() => setShowScreenshotModal(false)} className="p-2 hover:bg-stone-50 rounded-full text-stone-400 hover:text-stone-900 transition-colors">
                  <X size={20} />
                </button>
             </div>
             ...
             <div 
                className={`
                   border-2 border-dashed rounded-3xl p-10 flex flex-col items-center justify-center text-center transition-all duration-200
                   ${dragActive ? 'border-pink-500 bg-pink-50' : 'border-stone-200 bg-stone-50 hover:border-stone-300'}
                `}
                ...
                onDrop={handleDrop}
             >
                <div className="w-16 h-16 bg-white rounded-2xl shadow-sm flex items-center justify-center mb-4 text-pink-500">
                   {isUploading ? <div className="w-6 h-6 border-2 border-stone-200 border-t-pink-500 rounded-full animate-spin" /> : <Upload size={32} />}
                </div>
                <h3 className="text-lg font-bold text-stone-900 mb-2">Upload your screenshot</h3>
                <p className="text-stone-500 text-sm mb-6 max-w-xs">Drag and drop your image here, or click to browse. The AI will analyze the layout and structure.</p>
                ...
                <button 
                  ...
                  className="bg-black text-white px-6 py-3 rounded-full font-medium hover:bg-stone-800 transition-colors shadow-lg shadow-stone-200"
                >
                  Browse Files
                </button>
             </div>
          ...
          <div className="relative bg-white rounded-[2rem] shadow-2xl w-full max-w-lg p-8 animate-in zoom-in-95 fade-in duration-300">
             <div className="flex justify-between items-center mb-6">
                <div className="flex items-center gap-3">
                   ...
                   <h2 className="text-xl font-bold text-stone-900 font-display">Figma to Email</h2>
                </div>
                <button onClick={() => setShowFigmaModal(false)} className="p-2 hover:bg-stone-50 rounded-full text-stone-400 hover:text-stone-900 transition-colors">
                  <X size={20} />
                </button>
             </div>
             
             <p className="text-stone-500 mb-6">
               Paste your Figma file URL below. We use the Figma API to inspect the frame structure and auto-generate the HTML email code.
             </p>
             ...
             <div className="space-y-4">
               <div>
                  ...
                  <div className="relative">
                    <Link2 className="absolute left-4 top-1/2 -translate-y-1/2 text-stone-400" size={18} />
                    <input 
                      type="url"
                      placeholder="https://www.figma.com/file/..."
                      className="w-full bg-stone-50 border border-stone-200 rounded-2xl pl-11 pr-4 py-4 text-stone-900 placeholder-stone-400 focus:outline-none focus:ring-2 focus:ring-black/5 focus:border-stone-400 transition-all"
                      value={figmaUrl}
               ...
               <button 
                 onClick={() => setShowFigmaModal(false)}
                 className="w-full bg-black text-white py-4 rounded-2xl font-bold hover:bg-stone-800 transition-colors shadow-lg shadow-stone-200 mt-4"
               >
                 Save Link
               </button>
             </div>
          ...
          <div className="relative bg-white rounded-[2rem] shadow-2xl w-full max-w-lg p-8 animate-in zoom-in-95 fade-in duration-300">
             <div className="flex justify-between items-center mb-6">
                <div className="flex items-center gap-3">
                   ...
                   <h2 className="text-xl font-bold text-stone-900 font-display">YouTube to Email</h2>
                </div>
                <button onClick={() => setShowYoutubeModal(false)} className="p-2 hover:bg-stone-50 rounded-full text-stone-400 hover:text-stone-900 transition-colors">
                  <X size={20} />
                </button>
             </div>
             
             <p className="text-stone-500 mb-6">
               Paste a YouTube video URL below. We'll analyze the content and generate an email campaign based on the video's topic.
             </p>
               ...
               <div>
                  ...
                  <div className="relative">
                    ...
                    <input 
                      ...
                      className="w-full bg-stone-50 border border-stone-200 rounded-2xl pl-11 pr-4 py-4 text-stone-900 placeholder-stone-400 focus:outline-none focus:ring-2 focus:ring-red-500/10 focus:border-red-400 transition-all"
                      value={youtubeUrl}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-[2.5rem] shadow-2xl w-full max-w-2xl min-h-[500px] p-10 animate-in zoom-in-95 fade-in duration-300 flex flex-col items-center justify-between">
            ...
            <div className="absolute top-8 left-8">
               <PromptBuilderIcon className="w-12 h-12" color="#000000" />
            </div>

            <button onClick={() => setShowPromptBuilder(false)} className="absolute top-8 right-8 p-2 hover:bg-stone-50 rounded-full text-stone-400 hover:text-stone-900 transition-colors">
              <X size={24} />
            </button>
            ...
            <div className="w-full flex-grow flex flex-col items-center justify-center mt-12 mb-20 text-center">
               {!generatedPrompt && !isBuildingPrompt && (
                 <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                   <h2 className="text-3xl font-bold text-stone-900 font-display mb-4">Prompt Builder</h2>
                   <p className="text-lg text-stone-500 max-w-md mx-auto">Tell me what you want to create, and I'll architect the perfect engineering prompt for you.</p>
                 </div>
               )}

               {isBuildingPrompt && (
                 <div className="w-full max-w-lg space-y-4 animate-in fade-in duration-500">
                    {/* Floating Skeleton UI */}
                    ...
                    <div className="h-4 bg-stone-100 rounded-full w-5/6 mx-auto animate-pulse delay-150"></div>
                    <div className="h-4 bg-stone-100 rounded-full w-4/5 mx-auto animate-pulse delay-200"></div>
                 </div>
               )}

               {generatedPrompt && !isBuildingPrompt && (
                 <div className="w-full max-w-xl animate-in fade-in zoom-in-95 duration-500">
                    <div className="text-left bg-white/50 backdrop-blur-sm p-2 rounded-2xl">
                       <p className="text-stone-800 text-lg font-medium leading-relaxed whitespace-pre-wrap">{generatedPrompt}</p>
                    </div>
                    ...
                    <div className="flex justify-center gap-6 mt-8">
                        <button 
                            ...
                            className="flex flex-col items-center gap-2 text-stone-400 hover:text-stone-900 transition-colors group"
                        >
                            <div className="w-10 h-10 rounded-full bg-stone-50 group-hover:bg-stone-100 flex items-center justify-center">
                                <Copy size={18} />
                            </div>
                        ...
                        <button 
                            onClick={handleUseBuiltPrompt}
                            className="flex flex-col items-center gap-2 text-stone-400 hover:text-blue-600 transition-colors group"
                        >
                            <div className="w-10 h-10 rounded-full bg-blue-50 text-blue-500 group-hover:bg-blue-100 flex items-center justify-center">
                                <Check size={18} />
                            </div>
                            <span className="text-xs font-medium">Use Prompt</span>
                        </button>
                    </div>
                 </div>
               )}
            ...
            {/* Bottom Input Area */}
            <div className="absolute bottom-10 left-0 right-0 px-10 flex justify-center w-full">
               <div className="w-full max-w-xl relative bg-white shadow-xl shadow-stone-200/50 rounded-full border border-stone-100 p-2 pl-6 flex items-center transition-all focus-within:ring-2 focus-within:ring-stone-100 focus-within:border-stone-300">
                  <input 
                    type="text" 
                    value={builderInput}
                    onChange={(e) => setBuilderInput(e.target.value)}
                    onKeyDown={(e) => e.key === 'Enter' && handleBuildPrompt()}
                    placeholder="e.g. A follow up email to a client..."
                    className="flex-grow bg-transparent border-none outline-none text-stone-800 placeholder-stone-400"
                  />
                  <button 
                    onClick={handleBuildPrompt}
                    disabled={!builderInput.trim() || isBuildingPrompt}
                    className="bg-black text-white rounded-full px-6 py-3 hover:bg-stone-800 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 shadow-lg shadow-black/20"
                  >
